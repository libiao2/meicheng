...
class AddFood extends Component {
  ...
  componentWillMount() {
    ...
    if(parmars.id) {
      ...
      if(Taro.getStorageSync('addFoodInfo') != '') {
        console.log('||||||||', Taro.getStorageSync('shopFoodChooseImg'))
        ...
        this.setState({
          ...
          chooseShopImgList: Taro.getStorageSync('shopFoodChooseImg'),
          id: Taro.getStorageSync('changeAddFoodId'),
        })
  ...
  /// 获取需要修改的内容
  getChangeData(id) {
    const { foodType, serveList } = this.state;
    api.post('/product/toUpdate',{id}).then(res => {
      ...
      if(res.data.code == 200) {
        ...
        let chooseFoodType = obj.productType.split(',');
        let foodTypeCopy = [...foodType];
        foodTypeCopy.forEach(item => {
          chooseFoodType.forEach(item2 => {
            if(item.label == item2) {
              item.isChoose = true;
            }
          })
        ...
        let chooseServeList = obj.serviceSupport.split(',');
        let serveListCopy = [...serveList];
        serveListCopy.forEach(item => {
          chooseServeList.forEach(item2 => {
            if(item.label == item2) {
              item.isChoose = true;
            }
          })
  ...
  saveData() {
    ...
    Taro.setStorageSync('shopFoodChooseImg', chooseShopImgList);
    Taro.setStorageSync('changeAddFoodId', id);
  }
  ...
  deleteChooseImg(index) {
    const { chooseShopImgList } = this.state;
    let newList = [...chooseShopImgList];
  ...
  submitClick() {
    ...
    let obj = {
      ...
      categoryId: 1,
      verifyStatus: 0,
      picList: [...picList, ...chooseShopImgList],
      name,
      city,
      address,
      openTimeStart,
  ...
  render () {
    const { serveList, chooseShopImgList } = this.state;
    return (
        ...
        <View>
            <AtForm>
              <View className='shopImgBox'>
                ...
                {
                  chooseShopImgList.length > 0 &&
                  <View className='chooseImgList'>
                    {
                      chooseShopImgList.map((item,index) => {
                        return (
