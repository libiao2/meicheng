...
class AddCoupon extends Component {
  ...
  deleteChooseImg(index) {
    const { chooseCouponImgList } = this.state;
    let newList = [...chooseCouponImgList];
  ...
  goAddFood() {
    ...
    let obj = {
      picList: [...chooseCouponImgList, ...picList],
      name,
      detail,
      note,
      description,
      ...
      <View className='container'>
        <AtForm>
          <View className='shopImgBox'>
            ...
            {
              chooseCouponImgList.length > 0 &&
              <View className='chooseImgList'>
                {
                  chooseCouponImgList.map((item,index) => {
                    return (
