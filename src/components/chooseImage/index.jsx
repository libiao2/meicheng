...
class ChooseImage extends Component {
    ...
    if(doType==='remove'){
      ...
      v.map((item, index)=>{
        if (item.url.indexOf(".pdf") > -1 || item.url.indexOf(".PDF") > -1) {
          v[index].url = require("../../image/car.png")
        }
      })
      this.setState(()=>{
        return ({
  ...
  toUpload = () => {
    const { files } = this.state
    if(files.length>0){
      this.props.onFilesValue(files)  /// 传图片到父组件
      const rootUrl = 'https://api.uslife360.com/uslife/app' // 服务器地址
      ...
      success: (resp) => {
         ...
         //图片上传成功，图片上传成功的变量+1
          ...
          if(resultData.code == "200"){
            this.props.returnImgSrc(resultData.data)
            success++;
            this.setState((prevState)=>{
              console.log('============', prevState)
              let oldUpload = prevState.upLoadImg == undefined ? [] : prevState.upLoadImg
  ...
  render() {
    const { showUploadBtn, files } =this.state
